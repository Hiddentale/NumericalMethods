...
def LU_decomposition_old(matrix: NDArray) -> NDArray:
    ...
    # Construct empty L and U matrices with same dimensions as given matrix
    ...
    # Calculate both L and U matrices
    for i in range(length_of_given_square_matrix):
        ...
        for j in range(i, length_of_given_square_matrix):
            ...
            for k in range(i):
                sum_of_previous_calculated_elements += (
                    lower_triangular[i][k] * upper_triangular[k][j]
                )
        ...
        for m in range(i + 1, length_of_given_square_matrix):
            ...
            for k in range(i):
                sum_of_previous_calculated_elements += (
                    lower_triangular[m][k] * upper_triangular[k][i]
                )
            lower_triangular[m][i] = (
                matrix[m][i] - sum_of_previous_calculated_elements
...
# matrix = np.array([[6, 18, 3], [2, 12, 1], [4, 15, 3]])
